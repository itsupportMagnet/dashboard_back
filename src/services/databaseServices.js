...
export const getQuotes = async () => {
  const query = "SELECT `quoteID`, `modeOfOperation`, `pol`, `deliveryAddress`, `equipment`, `containerSize`, `containerType`, `weight`, `commodity`, `hazardous`, `bonded`, `loadType`, `date` FROM quotes";
  return pool.query(query)
    .then(rows => rows[0])
    .catch(error => {
