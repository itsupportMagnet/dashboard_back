...
export const changeCarrierInfoById = async (carrierId, name, contact, mc, dot, SCAC, EIN, form1099, insurance, address, city, zipcode, state, country, doct, carrierType, carrierPhone, carrierEmail, idCompany, ports) => {
  const query = `UPDATE carriers SET name = ? , contact = ?, mc = ? , dot = ? , SCAC = ?, EIN = ?, \`1099\` = ?, insurance = ? , address = ? , city = ?, zipcode = ? , state = ? , country = ?, DOCT = ?, type = ?, phone_number = ? , contact_email = ? WHERE id_carrier = ? , company_userID = ? AND ports = ?`;
  return pool.query(query, [name, contact, mc, dot, SCAC, EIN, form1099, insurance, address, city, zipcode, state, country, doct, carrierType, carrierPhone, carrierEmail, carrierId, idCompany, ports])
    .then()
    .catch(error => {
      console.error("Error on SQL: " + error);
      throw error;
